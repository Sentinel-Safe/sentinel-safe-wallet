...
def submit_signature(tx_id: str, signer_name: str, signer_info: Dict[str, str], tx_hash: str):
    ...
    if response.status_code == 200:
        ...
        if 'success' in result and result['success']:
            print(f"   ✅ Signed successfully ({result['current_signatures']}/{result['required_signatures']} signatures)")
        else:
...
def check_status(tx_id: str):
    ...
    if response.status_code == 200:
        ...
        print(f"   Signatures collected: {status['signatures_collected']}/{status['required_signatures']}")
...
def main():
    ...
    try:
        ...
        if len(signers_list) < 4:
            print(f"❌ Error: Need at least 4 signers, but only {len(signers_list)} found")
            return 1
